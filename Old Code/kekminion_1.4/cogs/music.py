        ...
        if (ctx.author.voice):
            ...
            if not voice.is_playing(
                ...
                queue.append(str(
                    ...
                    songID))  #Lets user know the command has found the song.
                embed = discord.Embed(
                    title="Playing ♪... ",
                    description=
                    f"Title: [`{songTitle}`](https://www.youtube.com/watch?v={songID}) \nDuration: `{songLength}` \nUploaded by: [`{str(uploaderSong)}`](https://www.youtube.com/channel/{uploaderIDSong})",
                    color=0x44ff44)
                if songID is None:  #If no ID, then default thumbnail.
                    ...
                    songID = info['id']
                embed.set_image(
                    url=f"https://img.youtube.com/vi/{songID}/0.jpg".format(
                        songID=songID))
                await ctx.send(embed=embed)
                isSongActive = True
            elif voice.is_playing(
            ) and ctx.voice_client:  #Check if song is currently playing and if bot is in vc.
                queue.append(str("https://www.youtube.com/watch?v=" + songID))
                print(queue)
                embed = discord.Embed(
                    title="",
                    description=f"A song is already playing!",
                    color=0xff3c28)
                await ctx.reply(embed=embed, mention_author=False)
                print(f"Play command used, but a song was already playing.")
            else:
                embed = discord.Embed(
                    title="",
...
# Stops the music from playing.
    ...
    async def stop(self, ctx):
        ...
        if voice.is_playing() and ctx.voice_client:
            voice.stop()
            await ctx.reply('Stopping...', mention_author=False)
...
# Shows information of the current song.
    ...
    async def now_playing_test(self, ctx):
        if ctx.voice_client:
            if songSeconds >= 3600:
                songLength = time.strftime("%H:%M:%S",
                                           time.gmtime(songSeconds))
